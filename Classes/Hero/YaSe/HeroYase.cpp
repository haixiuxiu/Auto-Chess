...
bool HeroYase::init(Ecamp camp, Ref* scene)
{
    ...
    float scaleFactorY = scalingWidth / this->getContentSize().height;
    ...
    this->setAttackScope(warriorAttackScope);
    ...
    //血条、蓝条
    this->_hpBar->setScaleY(1.2f);
    this->_hpBar->setScaleX(0.5f);
    this->addChild(this->_hpBar);
    this->_hpBar->setPosition(Vec2(this->getContentSize().width / 2, this->getContentSize().height + 35));

    this->_energyBar->setScaleY(1.2f);
    this->_energyBar->setScaleX(0.5f);
    this->addChild(this->_energyBar);
    this->_energyBar->setPosition(Vec2(this->getContentSize().width / 2, this->getContentSize().height + 30));

    std::function<void(float)> upDateHpAndEnergyPos = [this](float) {
        if (getHpBar() != nullptr && getEnergyBar() != nullptr)
        {
            this->getHpBar()->setPosition(Vec2(50, this->getContentSize().height + 38));
            this->getEnergyBar()->setPosition(Vec2(50, this->getContentSize().height + 30));
            label->setPosition(Vec2(50, this->getContentSize().height - 10 - label->getContentSize().height + 30));
        }
    };
